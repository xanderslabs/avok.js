...
//
// Type errors hide in TEST files. vitest does not typecheck, so if `test` does not call tsc then
// `pnpm typecheck` is the only thing that reads them — and a command nobody runs stays red.
// A `--filter` that matches nothing has the same shape one level down: the step passes having run
// nothing.
//
...
if (!test.includes("pnpm typecheck")) {
  fail(
    "`test` no longer runs `pnpm typecheck`.\n" +
      ...
      "  command anyone actually runs — which is exactly how they accrue unnoticed.",
  );
}
...
if (build < 0 || build > typecheck) {
  fail(
    "`test` must run `pnpm build` BEFORE `pnpm typecheck`.\n" +
      "  Cross-package types resolve through built dist/: @avokjs/core publishes its types as\n" +
      "  packages/core/dist/index.d.ts, which is what react and react-native read. There are no\n" +
      "  `paths` mappings, so the build is load-bearing. Without it typecheck passes on a warm machine\n" +
      "  and fails on a fresh clone.",
  );
}
