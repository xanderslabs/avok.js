...
import { produceEvmKey, type SecretContainer } from "./crypto/container.js";
...
export async function createWallet(args: {
  ...
  /** Operator/app label for the passkey display label (e.g. rpId). NEVER a user-identifying name. */
  networkName: string;
  now?: Date;
}): Promise<BirthResult> {
...
export async function addPasskey(args: {
  ...
  // access slot's id. Computed HERE because it needs K, and K must not leave the wallet module — the caller
...
/** Raw key material. Usable in MetaMask. Deliberately NOT a 24-word phrase: no standard derivation
 *  path reproduces our HKDF chain, so a phrase would look restorable and restore nothing. */
export type ExportedWallet = { evm: Hex };

/** Atomic export of the container as the raw private key. Requires explicit confirmation —
...
export async function exportWallet(args: {
  ...
  // into MetaMask. That string is immutable and CANNOT be wiped — inherent to letting a human read
  // a key. The container bytes are still zeroed by withDecryptedContainer's `finally`.
