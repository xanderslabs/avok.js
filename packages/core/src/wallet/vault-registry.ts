 ...
 * vault-from-marker fallback, the shared-origin origin's wallet-state, and the shared-origin authorize
 * popup.
 *
 ...
 * nothing at all about the wallet. NEVER collapse the two with a `catch { return null }`: that makes
 * an orphan indistinguishable from an RPC blip, and orphans become invisible. Only a secondary ever
 * reaches this reader — a birth credential derives K from its own PRF.
 *
  ...
  return {
    async getAccessSlot(addr, slotId) {
      ...
      try {
        ...
        //
        // Reading the zero-data throw as "the network failed" makes every orphan look like a blip,
