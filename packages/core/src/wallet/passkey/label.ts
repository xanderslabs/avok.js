 ...
 *  ciphertext from the chain that actually holds it, not from its own app anchor. Every resolution
 *  path finds the blob via evm + anchorChain + credentialId. The anchor is a WRITE-side default
 *  (where THIS app stores a NEW secondary); reads follow this marker. Always an EVM numeric
 *  chainId — `resolveAnchorChain` rejects anything else. */
