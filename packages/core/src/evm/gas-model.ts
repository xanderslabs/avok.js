...
/**
 * Send-time EIP-1559 policy for the client native-gas path.
 *
 * `NATIVE_GAS_FEE_MUL` multiplies the BASE FEE to buy spike headroom on the maxFee CEILING — which
 * EIP-1559 refunds, so headroom there is free. `NATIVE_GAS_TIP_MUL` multiplies the tip the CHAIN
 ...
 */
export const NATIVE_GAS_FEE_MUL = 2n;
export const NATIVE_GAS_TIP_MUL = 1n;
export function nativeGasFees(
  suggestedTip: bigint,
  baseFee: bigint,
): { maxFeePerGas: bigint; maxPriorityFeePerGas: bigint } {
  const maxPriorityFeePerGas = suggestedTip * NATIVE_GAS_TIP_MUL;
  return { maxFeePerGas: baseFee * NATIVE_GAS_FEE_MUL + maxPriorityFeePerGas, maxPriorityFeePerGas };
}

/**
 * The price per gas a native-gas transaction is EXPECTED TO PAY: the chain's BASE FEE plus the tip it
 * bids on top. `nativeGasFees().maxFeePerGas` is a CEILING, not a price — EIP-1559 charges
 ...
 */
export function nativeGasEffectiveGasPrice(suggestedTip: bigint, baseFee: bigint): bigint {
  return baseFee + suggestedTip * NATIVE_GAS_TIP_MUL;
}
...
/**
 * Full-transaction gas for the NATIVE-GAS rail — and it is EXACT, not an approximation.
 *
 * Native-gas sends precisely the transaction this simulates: the wallet calling `execute` on itself. So
 ...
 */
export async function nativeGasGasEstimate(args: {
  rpc: RpcClient;
  walletAddress: Address;
  implementation: Address;
  calls: Call[];
