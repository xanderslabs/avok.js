 ...
 * viem's `createPaymasterClient` so the rest of core never imports viem's account-abstraction surface
 * directly, and so the EntryPoint defaults to Avok's v0.9 target. The per-send fee token rides in
...
export interface Paymaster7677Options {
  ...
  transport?: Transport;
  /** EntryPoint the paymaster serves; defaults to the v0.9 canonical singleton. */
...
export function createPaymaster7677(opts: Paymaster7677Options): Paymaster7677 {
  ...
  const defaultEntryPoint = opts.entryPointAddress ?? entryPoint09Address;
