...
export interface ExecutionContext {
  chainId: number;
  /** The token the user REPAYS the paymaster in. Present ⇒ sponsored. Null on a sponsored batch means
   *  pure sponsorship: nothing is repaid (see `sponsored`). */
  feeToken?: Address | null;
  /** Sponsorship asked for WITHOUT a repayment token — the paymaster pays the gas outright and the
   *  user is charged nothing. This is the stock ERC-7677 verifying-paymaster case (Pimlico, Alchemy,
   *  Coinbase), and it is the whole point of the rail for an app onboarding users who hold no funds
   *  at all. Kept distinct from `feeToken` because "who pays" and "in what" are different questions:
   *  a fee token implies sponsorship, but sponsorship does not imply a fee token. */
  sponsored?: boolean;
...
export function railFromContext(ctx: ExecutionContext): Rail {
  return ctx.sponsored || ctx.feeToken ? "sponsored" : "native-gas";
}
...
export interface ResolvedBatch {
  ...
  /** The native-gas intent nonce. (The 4337 sponsored rail uses the EntryPoint's own 2D nonce instead,
  ...
  fee?: FeeBreakdown;
  /**
   * NATIVE-GAS ONLY — what this transaction is expected to cost the wallet in the chain's NATIVE gas
   * asset. Nothing signs it (native-gas commits no fee call; the chain debits the wallet at inclusion),
   * so it is an ESTIMATE, not a promise — see NativeFeeEstimate.
   *
   * It exists because "native-gas" is not an excuse to show the user no number at all.
   */
...
/**
 * The expected native-gas cost of a NATIVE-GAS transaction.
 *
...
export interface SimulationResult {
  ...
  fee?: FeeBreakdown;
  /** Native-gas only — the ESTIMATED native cost. Never signed. */
  nativeFee?: NativeFeeEstimate;
...
export interface Receipt {
  /** txHash (native-gas) or intent id (sponsored). */
  id: string;
  rail: Rail;
