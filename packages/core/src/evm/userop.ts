...
/** Builds a v0.9 UserOperation whose `callData` is the standard ERC-7821 `execute(MODE_BATCH, calls)`. */
...
/** The v0.9 userOpHash — an EIP-712 hash the account signs directly (viem's canonical computation).
 *  The hash binds the EntryPoint address through the EIP-712 domain, so it MUST use the same v0.9
 *  singleton the contract's ENTRY_POINT points at, or validateUserOp recovers a different signer. */
