...
export function createViemVaultReader(rpc: RpcClient): VaultReader {
  return {
    async getAccessSlot(address: Address, slotId: Hex) {
      // This reader and the wallet module's registry reader MUST classify identically. Two readers that
      ...
      // IDENTICALLY to the wallet module's registry reader (wallet/vault-registry.ts): inactive, empty, or missing
