 ...
 * (already-signed) v0.9 UserOp directly (no viem SmartAccount preparation), so the Avok
 * `Connection`/`userop.ts` stays the single source of the UserOp and its signature. Passes the v0.9
...
export interface BundlerOptions {
  ...
  transport?: Transport;
  /** EntryPoint the bundler serves; defaults to the v0.9 canonical singleton. */
  entryPointAddress?: Address;
}

/** A fully-built v0.9 UserOperation ready to estimate/submit. */
export type AvokUserOperation = UserOperation<"0.9">;
...
export function createBundler(opts: BundlerOptions): Bundler {
  ...
  const entryPointAddress = opts.entryPointAddress ?? entryPoint09Address;
