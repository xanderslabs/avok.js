...
export type ChannelRequest =
  // `credentialId` MUST be carried down to the popup. Without it the browser cannot constrain the
  // assertion, so the user is asked to pick a passkey on EVERY signature. The app holds it
  // (connect() returned it) and sends it here. This is the canonical statement of that rule; the
  // other credentialId sites point back here.
