...
export function createWebChannel({ authOrigin }: { authOrigin: string }): SigningChannel {
  ...
  return {
    open(req: ChannelRequest): Promise<ChannelResult> {
      ...
      // posts `ready`, then dispatches on the request kind (authorize | sign). NOTHING TRAVELS IN THE
      // URL: the web channel correlates request and reply by postMessage channel binding,
      // event.source === popup.
