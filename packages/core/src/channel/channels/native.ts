 ...
 *
 * That rests on PRF evaluating inside the tab, which is measured rather than assumed. The
 * measurement, its date, and how to re-run it live in ONE place: `@avokjs/react-native`'s
 * VERIFICATION.md §3b. Do not restate the result here — update it there.
 *
 * RFC 8252 §6 RECOMMENDS the in-app browser tab for exactly this, and §8.12 says native apps
 * "MUST NOT use embedded user-agents" — the WebView alternative is prohibited, not merely worse.
 * (The platform APIs are named in Appendix B.1/B.2, and predate ASWebAuthenticationSession: the RFC
 * says SFAuthenticationSession, its direct predecessor, and Android Custom Tabs.)
 *
 ...
 *     an oversized URL is dropped silently (Chromium replaces it with an empty, invalid URL) and
 *     looks exactly like a cancellation.
 *
