...
export interface FullAvokClient extends UseOnlyAvokClient {
  ...
  /**
   * Export the wallet's key — the EVM secp256k1 private key. K IS that key, so this single key
   * restores the whole wallet. Own-origin self-custody only. Raw key, never a phrase (no standard
   * derivation reproduces the HKDF chain).
   */
  ...
  function accessCtx(): AccessCtx {
    return {
      ...
      assertCanAffordAccessSlot: async (chainId: number, feeToken?: Address | null) => {
        ...
        // native-gas simulation could never produce it, and a token-paying user was measured against a
        ...
        // Native-gas: native gas, paid by the user directly.
        const rpc = resolveRpc(config, chainId);
      ...
      // NATIVE-GAS: one gesture — resolve without the key, seal AND sign inside a single scope,
