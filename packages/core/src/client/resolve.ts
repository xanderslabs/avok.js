 ...
 * Covers: delegation authorization + userCalls (+ native-gas native-fee estimate).
 *
...
export async function leanResolve(args: LeanResolveArgs): Promise<ResolvedBatch> {
  ...
  //    price here. NATIVE-GAS signs no fee either, but the user still gets a native-cost ESTIMATE.
  if (rail === "native-gas") {
