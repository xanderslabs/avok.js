/**
 * Thrown when a per-send `feeToken` is NOT a supported fee token on the chain the transaction
 * executes on.
 *
 * Fee tokens are ERC-20 *addresses*, and an address is chain-specific — USDC on Base is a
...
export class UnsupportedFeeTokenError extends Error {
  /** The offending fee-token address. */
  readonly token: string;
  /** The target chain the token was rejected on, as a namespaced id (`eip155:<id>`). */
  ...
  constructor(token: string, chain: number | string) {
    ...
    super(
      `Fee token ${token} is not a supported fee token on ${chainLabel}. Fee-token addresses are ` +
        `chain-specific — pass a token supported on this chain, or omit the fee token (feeToken: null) to native-gas.`,
    );
