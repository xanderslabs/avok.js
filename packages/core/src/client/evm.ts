...
export type TxOpts = {
  chainId?: number;
  /** The ERC-20 the user REPAYS the paymaster in. Setting it asks for sponsorship. */
  feeToken?: Address | null;
  /** Ask for sponsorship WITHOUT a repayment token: the paymaster pays and the user is charged
   *  nothing. Redundant alongside `feeToken`, which already asks. */
  sponsored?: boolean;
};

/** A supported EVM fee token for a chain. */
...
export interface EvmNamespace {
  /** The fee tokens the REGISTRY knows for this chain — a static catalogue of what Avok can describe.
   *  It is NOT a promise the configured paymaster accepts them: under ERC-7677 that answer is
   *  transaction-scoped and provider-specific (`pm_getPaymasterStubData` accepts or rejects the actual
   *  userOp), so there is no honest ahead-of-time "which tokens" list to return. A misconfigured or
   *  unreachable rail surfaces as `SponsorshipUnavailableError` before signing. */
   ...
   * native-gas receipt is `submitted` (broadcast, not mined) and a sponsored receipt is `pending` with
...
export function createEvmNamespace(config: ClientConfig): EvmNamespace {
  ...
  /** The error a sponsorship request gets when no rail exists. Reports which side is missing. */
  function noRail(chainId: number): SponsorshipUnavailableError {
    return SponsorshipUnavailableError.evm(chainId, {
  ...
  /** What THIS send resolved to: which rail, and the token the user repays the paymaster in. */
  type Sponsorship = { sponsored: boolean; feeToken: Address | null };
  const NATIVE_GAS: Sponsorship = { sponsored: false, feeToken: null };

  /**
   * TWO WAYS TO ASK FOR SPONSORSHIP, because "who pays" and "in what" are separate questions:
   *
   *   { feeToken: USDC } → sponsored; the paymaster fronts the gas and the user REPAYS in USDC.
   *   { sponsored: true } → sponsored; the paymaster pays outright and the user repays NOTHING.
   *   { }                → native-gas. Sponsorship is never inferred.
   *
   * The second is the stock ERC-7677 verifying paymaster (Pimlico, Alchemy, Coinbase) and the reason
   * the rail exists at all for onboarding: a brand-new user holds no gas AND no fee token, so a design
   * that can only reach the sponsored rail through a fee token cannot serve them. It also cannot be
   * reached on any chain whose registry lists no fee tokens.
   *
   * ASKING IS BINARY: it is served or it throws. There is no degrade-to-native-gas mode, because the
   * two things a degrade produces are both wrong. A user holding only the fee token gets an
   * insufficient-funds error naming a native balance, hiding the endpoint that actually caused it. A
   * user holding a little native has their OWN funds spent on a send the app meant to sponsor. A
   * config gap must not reach anyone as a balance problem.
   *
   * Fee-token is PER-SEND — there is no client-level default (SPEC §5).
   */
  function resolveSponsorship(chainId: number, opts?: TxOpts): Sponsorship {
    const token = opts && "feeToken" in opts ? (opts.feeToken ?? null) : null;
    if (!token && !opts?.sponsored) return NATIVE_GAS;

    // No 4337 infra on this deployment (no bundler+paymaster) means there is no rail to serve the ask.
    // Throw HERE: before the batch is built, and long before the passkey ceremony. See the note above
    // for why there is nothing to fall back to.
    if (!canSponsor()) throw noRail(chainId);
    ...
    if (token && !feeTokens(chainId).some((t) => t.address.toLowerCase() === token.toLowerCase())) {
      throw new UnsupportedFeeTokenError(token, chainId);
    }
    return { sponsored: true, feeToken: token };
  }
  ...
  return {
    ...
    async simulate(calls, opts?) {
      ...
      const batch = await buildBatch(calls, chain, rpc, resolveSponsorship(chainId, opts));
    ...
    async send(input, opts?) {
      ...
      if (Array.isArray(input)) {
        ...
        batch = await buildBatch(input, chain, rpc, resolveSponsorship(chainId, opts));
      } else {
        batch = input.batch;
      ...
      //
      // 2× headroom, not 1.2×: native-gas is the one rail nobody re-estimates downstream (the relayer
      ...
      const commonFields = {
        ...
        nonce: txNonce,
        gas: gasLimit,
        ...nativeGasFees(suggestedTip, baseFee),
      };
      ...
      return { id: txHash, rail: "native-gas", status: "submitted", txHash, chainId: batch.chainId };
    },
  };
}
