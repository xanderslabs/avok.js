...
export interface AccessCtx {
   ...
   * the balance that will actually pay: native gas in native-gas, the fee TOKEN when one is passed. The
   ...
   * across a network round-trip. Native-gas needs none of this: its signature is built entirely
 ...
 *
 * The Signer verbs (signMessage, signTypedData, signSiwe, signAuthorization, signTransaction and
 * the composites) are inherited.
 *
...
export interface SelfCustodyConnection extends Connection {
  ...
  /**
   * Export the connection's key material so the user can back up the wallet. Returns `{ evm }`: the
   * raw private key, never a 24-word phrase. No standard derivation path reproduces the HKDF chain,
   * so a phrase would look restorable and restore nothing.
   */
