 ...
 * `token: null` means native gas (native-gas); otherwise it is the fee token (sponsored mode).
 */
...
export function createOwnOriginConnection(opts: {
  ...
  return {
    ...
    async create(): Promise<Account> {
      ...
      state = st;
      return { evm: { address: account.evm } };
    },

    async export(): Promise<ExportedWallet> {
      const st = requireState();
      // K IS the EVM key, so this one key is the whole wallet.
    ...
    async continue(): Promise<Account> {
      ...
      if (handle.kind === "primary") {
        ...
        return { evm: { address: state.evmAddress } };
      }
    ...
    async addPasskey(
      ...
      if (feeToken) {
        ...
        // A declined second gesture FAILS. It does not quietly fall back to native-gas: the caller asked
        // to pay in a token, and silently charging native gas instead is exactly the degrade that
        // a sponsorship request (which never degrades) exists to prevent — worse here, because the user may hold
        // no native gas at all.
        ...
        // NATIVE-GAS — ONE gesture. The signature is built entirely in-process from the same-length
