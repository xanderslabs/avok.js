 ...
 * never be live across a network round-trip (types.ts). Native-gas avoids all of it — its signature is
 * built in-process from a same-length probe — which is why it keeps its single gesture.
 *
 * MUTATION: make `addPasskey` ignore `feeToken` (always take the native-gas branch) and the rail and
...
describe("access-slot writes can be paid in a token", () => {
  it("routes through the paymaster when a fee token is given, and never when it is not", async () => {
    ...
    expect(nativeGas.seen.sponsorCalls).toBe(0);
  ...
  it("takes TWO key scopes when sponsored and ONE when paying native gas", async () => {
  ...
  it("fails cleanly when sponsorship is unavailable — it does not fall back to native-gas", async () => {
    // Silently charging native gas to a user who asked to pay in a token is the degrade that
    // a sponsorship request (which never degrades) exists to prevent, and it is worse here: the user may hold
    // no native gas at all, so the "fallback" is a transaction that cannot even be submitted.
    const ctx = recordingCtx();
