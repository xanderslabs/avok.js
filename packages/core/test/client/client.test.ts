...
describe("createAvokClient — native-gas send (D3)", () => {
  ...
  // Producing them through the connection's individual key-bound verbs would open two key scopes, so
  // a single "Send" would ask the user for a fingerprint TWICE. Beyond the annoyance, that teaches
  // people to approve prompts reflexively — the exact habit a malicious second prompt exploits.
  // These two tests fail the moment the scope count regresses.
  ...
  // `gas × maxFeePerGas` is the MOST a native-gas signature can cost, and on the shared-origin consent
  // screen it is the ONLY fee fact derivable from the signed bytes (native-gas commits no fee call, and
...
describe("createAvokClient — simulate (D3)", () => {
  it("native-gas: returns a SimulationResult without a gesture", async () => {
    const signTransaction = vi.fn();
    const connection = makeFakeConnection({ signTransaction });
    const rpc = makeFakeRpc({ delegated: false, nonce: 0 });
    const evm = createEvmNamespace({ connection, deps: { rpc, chain: TEST_CHAIN } });
    const sim = await evm.simulate([{ to: TO, value: 0n, data: "0x" }], { chainId: 10 });
    expect(sim.batch.rail).toBe("native-gas");
