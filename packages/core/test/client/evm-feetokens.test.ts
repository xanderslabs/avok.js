...
describe("resolveFeeToken chain validation", () => {
  ...
  it("a mismatched feeToken with no bundler/paymaster reports the ABSENT RAIL, not the token", async () => {
    // ORDER MATTERS, and this pins it: with no 4337 infra the rail check fires first, so a wrong-chain
    // token is never even consulted. Told "unsupported fee token", a developer goes and checks their
    // token address, which is the wrong hunt — the actual fault is that they configured no rail.
    const evm = createEvmNamespace({
      connection: makeFakeConnection(),
      deps: {
    ...
    const err = await evm.simulate([USER_CALL], { chainId: ARBITRUM, feeToken: BASE_USDC }).catch((e) => e);
    expect(err).toBeInstanceOf(SponsorshipUnavailableError);
    expect(err).not.toBeInstanceOf(UnsupportedFeeTokenError);
  });

  it("explicit feeToken:null forces native-gas even with a bundler/paymaster configured", async () => {
    const evm = createEvmNamespace({
      connection: makeFakeConnection(),
      paymasterUrl: "http://p",
      bundlerUrl: "http://b",
