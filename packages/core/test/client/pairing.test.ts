...
import { getChainProfile, type Call } from "../../src/evm/index.js";

// ERC-7821 execute(mode, executionData) wraps abi.encode(Call[]) — the shape the native-gas calldata carries.
const CALLS_PARAM = [
  {
    type: "tuple[]",
    components: [
...
describe("Enrolling the user's OWN second device (the same one ceremony)", () => {
  it("gives B its own passkey — and B is NOT logged in by the ceremony, because it received no key", async () => {
    // A second device of the user's own runs the SAME ceremony an independent domain runs, because
    // the passkey is the same kind of passkey. A never hands B the wallet key: B derives its own
    // wrapping key, A seals K under it and pays for the write, and B logs in afterwards like any
    // secondary — one extra passkey prompt, and K never touches the wire.
    ...
    expect(accountB.evm.address.toLowerCase()).toBe(accountA.evm.address.toLowerCase()); // same K
...
/**
 * A harness that plays the anchor chain for the FACADE path. Access-slot writes are now NATIVE-GAS
 * (SPEC §5), so the facade signs a native-gas transaction and broadcasts it via `rpc.sendRawTransaction`.
...
function facadeHarness(opts: { failWrite?: boolean } = {}) {
  ...
  const rpc = {
    ...
    sendRawTransaction: async (serialized: Hex): Promise<Hex> => {
      ...
      if (opts.failWrite) throw new Error("native-gas broadcast rejected by the node");
      const tx = parseTransaction(serialized) as { data?: Hex };
      const outer = decodeFunctionData({ abi: AvokCaliburABI, data: tx.data ?? "0x" });
 ...
 *  native-gas, so the holder's accessCtx carries the write through `rpc.sendRawTransaction`. */
...
describe("Passkey enrolment — through the client facade (accessCtx)", () => {
  it("the HOLDER's facade submits the addAccessSlot write, and the enrolled device then logs in", async () => {
    ...
    // HOLDER's accessCtx (evm.send → native-gas broadcast) is what carries the write. This drives that seam.
    ...
    expect(txId).toBe("0xnativegastx");

    // The write actually went out — decode the captured native-gas batch calls with the ABI.
    ...
    // And the enrolled device reaches the SAME wallet — byte-identical K ⇒ the same address.
