 ...
 *   - native-gas → status "submitted", with a REAL txHash (broadcast, not yet mined)
 ...
 *
 * `wait()` is the only thing allowed to produce "confirmed", and only when the chain (native-gas) or the
...
function clientWith(receiptSequence: (null | { success: boolean; transactionHash: Hex })[]) {
  ...
  const connection = {
    account: () => ({ evm: { address: WALLET as Address } }),
    status: () => true,
...
describe("a handed-off transaction is not a confirmed one", () => {
  it("wait() does NOT report confirmed while the bundler still has no receipt", async () => {
    ...
    // A hand-off to the bundler is not an inclusion. Reporting "confirmed" here, or linking a hash
    // the chain has never seen, tells the user their money moved when it may still fail.
