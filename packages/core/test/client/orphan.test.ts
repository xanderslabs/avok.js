...
describe("preflight: never mint a credential you cannot finish enrolling", () => {
  ...
  it("blocks the enrolment ceremony at the HOLDER, before the enroller mints anything", async () => {
    ...
    // Nothing was minted anywhere: the refusal happens BEFORE an INVITE is produced, so there is
    // nothing to hand an enroller. Refusing only after answering a request would mean the enroller
    // had already been contacted. No invite, no enrolment, no orphan.
  });
});
