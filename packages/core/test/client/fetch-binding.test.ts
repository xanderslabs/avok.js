...
const CHAIN_10_CONFIG = {
  chains: {
    10: { sponsor: "0x3333333333333333333333333333333333333333", supportedTokens: [], bufferBps: 1500, marginBps: 500 },
  },
};
...
describe("the default fetch is bound to the global", () => {
  it("a SPONSORED send does not die with 'Illegal invocation'", async () => {
    ...
    const connection = {
      account: () => ({ evm: { address: "0x1111111111111111111111111111111111111111" } }),
      status: () => true,
      signSend: vi.fn(),
      signSponsored: vi.fn(),
