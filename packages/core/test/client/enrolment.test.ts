...
describe("passkey enrolment (one ceremony, whoever the enroller is)", () => {
  ...
  it("the enroller sends its wrapping key BEFORE any confirmation — and that is the design", async () => {
    // Deliberate: the enroller does NOT gate W behind a SAS answer, because W alone is worthless.
    // W plus the on-chain blob yields K, and W by itself yields nothing. An attacker who intercepts
    // this wrap holds a key to a lock that is never built, because the holder compares digits before
    // publishing the blob.
    //
    // This test exists to make that explicit. Anyone who adds a confirmation gate here has misread
    // why the round is unnecessary.
    const vault = capturingVault();
    const { holder, enroller } = twoSides(vault);
    await holder.create();
