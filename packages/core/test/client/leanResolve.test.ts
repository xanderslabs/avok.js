...
describe("leanResolve", () => {
  ...
  it("native-gas: feeCalls is empty and rail is native-gas", async () => {
    ...
    const batch = await leanResolve({
      ...
      address: WALLET,
      userCalls: [USER_CALL],
      ctx: { chainId: 10 }, // no feeToken → native-gas
      nonce: 0n,
      deadline: 9_999_999_999n,
    });

    expect(batch.rail).toBe("native-gas");
    expect(batch.feeCalls).toHaveLength(0);
  ...
  it("native-gas rail when feeToken absent", async () => {
    const rpc = makeFakeRpc({ delegated: false, nonce: 0n });
    await expect(
      leanResolve({
        rpc,
    ...
    ).resolves.toMatchObject({ rail: "native-gas", feeCalls: [] });
  });
