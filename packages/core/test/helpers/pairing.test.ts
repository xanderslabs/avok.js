...
describe("runImportCeremony (the ENROLLER — the device getting a passkey)", () => {
  ...
  it("rejects when the user says the SAS does not match — and BURNS the credential it minted", async () => {
    // This is the cost of the two-round ceremony. The credential is minted inside mintAndWrap, which
    // runs BEFORE the user answers, so a mismatch leaves a real passkey behind. `reject` marks it
    // burned: W is scoped to (address, slotId) and
