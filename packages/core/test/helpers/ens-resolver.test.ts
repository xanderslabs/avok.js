...
// Name resolution is in scope on its own (VISION §6) and lives in helpers, so the ENS read path
// must stay proven standalone — with no registrar, no name package, and no wallet in the graph.
...
describe("createEnsResolver", () => {
  ...
  test("resolution needs NO registrar and NO parent — it resolves any .eth, suffix defaults to .eth", async () => {
    // WHY: an app that never mints a name still resolves vitalik.eth, with no registrar config.
    const svc = createEnsResolver({ chainId: 1, client: fakeClient() });
