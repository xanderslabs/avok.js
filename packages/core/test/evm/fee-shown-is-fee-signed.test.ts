 ...
 *
 * `simulateResolved` must NEVER re-price the fee for display from the SIMULATION's gas number.
 * `feeCalls` — what the signature actually covers, and what the sponsor moves — is priced from
 * `fullGasEstimate`, which also covers the EIP-7702 authorization intrinsic and the fee transfer
 * itself. Re-pricing puts two gas numbers and two fees inside ONE SimulationResult.
 *
 * On real hardware that shipped once: the app displayed a 0.001921 USDC network fee and the chain moved
