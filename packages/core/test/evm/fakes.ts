...
import type { RpcClient, SimCallResult, SimulateArgs, SimCall, StateOverride, ReadArgs } from "../../src/evm/rpc.js";
...
export class FakeRpcClient implements RpcClient {
  ...
  chainId() {
    return Promise.resolve(this.cfg.chainId ?? 10);
  }
  getCode(a: Address) {
    return Promise.resolve(this.cfg.code?.[a.toLowerCase()] ?? this.cfg.code?.[a] ?? "0x");
  }
  getTransactionCount(a: Address) {
    return Promise.resolve(this.cfg.nonces?.[a.toLowerCase()] ?? this.cfg.nonces?.[a] ?? 0);
  }
  simulateCalls(args: SimulateArgs) {
    this.lastSimulate = args;
    return Promise.resolve(
      this.cfg.simResults ??
        args.calls.map(() => ({ status: "success" as const, gasUsed: 21000n, returnData: "0x" as Hex })),
    );
  }
  ...
  getGasPrice() {
    return Promise.resolve(this.cfg.gasPrice ?? 1_000_000_000n);
  }
  getBaseFeePerGas() {
    return Promise.resolve(this.cfg.baseFee ?? 400_000_000n);
  }
  getMaxPriorityFeePerGas() {
    return Promise.resolve(this.cfg.suggestedTip ?? 600_000_000n);
  }
  getBalance() {
    return Promise.resolve(this.cfg.balance ?? 10n ** 18n);
  }
  readContract<T>(args: ReadArgs) {
    return Promise.resolve(this.cfg.reads?.[`${args.address}:${args.functionName}`] as T);
  }
  sendRawTransaction(s: Hex) {
    this.sent.push(s);
    return Promise.resolve(("0x" + "ab".repeat(32)) as Hex);
  }
  getTransactionReceipt(hash: Hex) {
    return Promise.resolve(this.cfg.receipts?.[hash] ?? null);
  }
}
