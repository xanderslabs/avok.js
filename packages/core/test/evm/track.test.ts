...
import { getReceiptStatus } from "../../src/evm/track.js";
import { FakeRpcClient } from "./fakes.js";

const TX = ("0x" + "ab".repeat(32)) as `0x${string}`;

test("native-gas: maps a mined receipt to confirmed/failed", async () => {
  const rpc = new FakeRpcClient({ receipts: { [TX]: { status: "success", transactionHash: TX } } });
  const out = await getReceiptStatus(
    { id: TX, rail: "native-gas", status: "submitted", txHash: TX, chainId: 10 },
    { rpc },
  );
...
test("native-gas: not yet mined stays submitted", async () => {
...
test("native-gas: receipt with no txHash is returned unchanged", async () => {
  const rpc = new FakeRpcClient();
  const receipt = { id: "tx_1", rail: "native-gas" as const, status: "submitted" as const, chainId: 10 };
