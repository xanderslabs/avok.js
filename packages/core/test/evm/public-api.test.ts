...
test("does NOT re-export gas-model internals or the unused viem smart-account wrapper", () => {
  // The raw native-gas gas constants + gas/price mechanics are internal (reached via pricing/resolve),
  ...
  for (const internal of [
    ...
    "NATIVE_GAS_FEE_MUL",
    "NATIVE_GAS_TIP_MUL",
    "nativeGasEffectiveGasPrice",
    "nativeGasGasEstimate",
    "decodeCalls",
    "toAvokSmartAccount",
...
test("public surface exposes CHAIN_PROFILES and send-builders", () => {
  ...
  expect(typeof api.buildNativeGasCalldata).toBe("function");
});
