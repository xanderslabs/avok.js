...
test("a TRANSPORT failure throws VaultUnreadableError — it is not evidence the slot is absent", async () => {
  // getAccessSlot must never swallow an error and return null. That makes an RPC outage
  // indistinguishable from a credential whose slot write never landed, so an orphan looks like a blip
  // and the caller tells the user to retry — forever. A read that FAILED says nothing about the
  // wallet, and it must say so.
  const throwingRpc = {
    readContract: async () => {
