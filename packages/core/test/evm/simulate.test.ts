...
import { simulateResolved, decodeCalls } from "../../src/evm/simulate.js";
...
test("sponsored: result.fee is the fee the batch COMMITTED TO — simulate never re-prices it", async () => {
  // simulate MUST NOT re-derive the fee from the simulation's own gas. The batch's `feeCalls` are
  // priced at resolve from fullGasEstimate (which also covers the 7702 authorization intrinsic and
  // the fee transfer), so a second derivation here yields a DIFFERENT number — and the app would show
  // it while the user signs the other one. On real hardware that gap was 0.001921 USDC displayed
  // against 0.004104 moved (Arc tx 0xf3f16510…b246).
  //
  // The rule: the fee is priced ONCE, carried on the batch, and merely surfaced here. The simulation
  // below returns deliberately different gas so any re-pricing shows up as a failure.
  const rpc = new FakeRpcClient({
...
test("native-gas: result.fee is undefined — a native-gas batch commits to no fee to surface", async () => {
  const rpc = new FakeRpcClient({ simResults: [{ status: "success", gasUsed: 70_000n, returnData: "0x" }] });
