...
import { estimateNativeFee } from "../../src/evm/pricing.js";
import { simulateResolved } from "../../src/evm/simulate.js";
import { AUTH_7702_GAS, BASE_TX_GAS, NATIVE_GAS_TIP_MUL, nativeGasFees } from "../../src/evm/gas-model.js";
import type { ResolvedBatch } from "../../src/evm/types.js";

/**
 * NATIVE-GAS MUST STILL DISCLOSE A FEE.
 *
 * The sponsored rail commits a fee to `feeCalls`, so the amount is exact and signed. Native-gas commits
 * nothing — the chain debits the wallet's native balance at inclusion — and the consent screen must
 * not answer that with no number at all ("paid in USDC at the current gas price"). "You will be
 * charged something" is not a fee disclosure.
 *
...
// execution. Verified on Arc: simulating execute([transfer]) returns 56,090 and the real native-gas
// transaction used exactly 56,090. The estimator must NOT add the intrinsic and an envelope constant
// ON TOP of this: that over-quotes the user by 51%.
const SIM_GAS = 56_090n;
// Three distinct numbers, satisfying the chain's identity gasPrice == baseFee + suggestedTip. The
// native-gas rail bids the SUGGESTED tip; bidding the whole gasPrice pays the base fee twice.
...
describe("native-gas native fee estimate", () => {
  it("IS the simulation — native-gas sends exactly the transaction it simulates", async () => {
    ...
    // the execute envelope, and native-gas broadcasts precisely that transaction.
    expect(est.gasUnits).toBe(SIM_GAS);
    expect(est.gasUnits).toBeLessThan(SIM_GAS + BASE_TX_GAS); // adding it again would over-quote
  ...
  it("does NOT quote the maxFeePerGas ceiling, which would over-state the fee", async () => {
    ...
    // `nativeGasFees().maxFeePerGas` carries spike headroom on the BASE fee; the chain refunds the
    // difference. Displaying the ceiling as the fee is a lie in the user's disfavour.
    const ceiling = nativeGasFees(SUGGESTED_TIP, BASE_FEE).maxFeePerGas;
  ...
  it("bids the tip the chain suggested — never the whole of eth_gasPrice", async () => {
    ...
    const { maxPriorityFeePerGas, maxFeePerGas } = nativeGasFees(SUGGESTED_TIP, BASE_FEE);

    expect(maxPriorityFeePerGas).toBe(SUGGESTED_TIP * NATIVE_GAS_TIP_MUL);
    expect(maxPriorityFeePerGas).not.toBe(GAS_PRICE); // bidding the whole gasPrice overbids
...
describe("simulateResolved surfaces the estimate it was given", () => {
  const nativeGasBatch: ResolvedBatch = {
    rail: "native-gas",
    chainId: 5042002,
    walletAddress: WALLET,
    feeCalls: [],
  ...
  it("passes batch.nativeFee through verbatim — it does not recompute from gasUsed", async () => {
    ...
    expect(sim.nativeFee).toEqual(nativeGasBatch.nativeFee);
    expect(sim.fee).toBeUndefined(); // native-gas commits no fee — there is nothing signed to show
  });
});
