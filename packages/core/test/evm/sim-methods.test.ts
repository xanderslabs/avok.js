...
test("buildNativeGasCalldata wraps feeCalls+userCalls in execute(MODE_BATCH,…) with fee-first ordering", () => {
  ...
  const decoded = decodeFunctionData({ abi: executeAbi, data: buildNativeGasCalldata(batch) });
  expect(decoded.functionName).toBe("execute");
