...
describe("createPaymaster7677", () => {
  ...
  it("defaults to the v0.9 EntryPoint and forwards the fee-token context to the RPC params", async () => {
    const calls: RpcCall[] = [];
    const pm = createPaymaster7677({ transport: fakePaymasterTransport(calls) });
  ...
  it("an explicit entryPointAddress overrides the v0.9 default", async () => {
    const calls: RpcCall[] = [];
    const pm = createPaymaster7677({ transport: fakePaymasterTransport(calls) });
