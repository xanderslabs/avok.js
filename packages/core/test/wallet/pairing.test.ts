 ...
 *
 * K NEVER TRAVELS. The enrolling side derives its own wrapping key and sends THAT; the wallet key
 * itself is never put on the wire. A key-shipping ceremony (sealContainer / unsealContainer /
 * PairGrant) would be a silent, total regression of that property, so this test fails if anyone
 * reintroduces key-transport on this channel.
 */
