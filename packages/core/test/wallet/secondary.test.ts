...
import { decodeUserHandle, deriveSlotId } from "../../src/wallet/passkey/label.js";
...
describe("addPasskey (secondary enrolment)", () => {
  it("wraps the EXISTING wallet key under the new credential's PRF", async () => {
    // The invariant: a secondary must reach the SAME K. A freshly derived key would be a new wallet.
