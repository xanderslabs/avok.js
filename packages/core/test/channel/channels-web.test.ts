...
import { createWebChannel } from "../../src/channel/channels/web.js";
import type { ChannelResult } from "../../src/channel/channels/port.js";
...
describe("createWebChannel", () => {
  ...
  it("opens the popup to the SAME auth origin ROOT for an authorize request (no per-kind URL)", async () => {
    ...
    fireMessage({ kind: "authorize", account: { evmAddress: "0xabc" }, proof: "0xsig" }, AUTH_ORIGIN);
    await promise;
