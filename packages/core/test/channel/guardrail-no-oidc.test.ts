...
import * as sharedOrigin from "../../src/channel/index.js";
...
describe("#8 guardrail: the auth-origin collapse holds", () => {
  ...
  it("the OIDC modules are gone from shared-origin/src", () => {
    for (const f of ["oidc-client.ts", "jwt-verify.ts"]) {
      expect(existsSync(resolve(ROOT, "packages/core/src/channel", f))).toBe(false);
    }
  });
  ...
  it("the channel carries credentialId, not sessionId, and returns no OIDC code", () => {
    ...
    const port = readFileSync(resolve(ROOT, "packages/core/src/channel/channels/port.ts"), "utf8")
      .split("\n")
      .filter((l) => {
        const t = l.trim();
