...
describe("continue() path selection", () => {
  ...
  it("a secondary whose blob is ABSENT from a READABLE chain is an ORPHAN, not a network failure", async () => {
    ...
    // NOT SlotUnreachableError: a chain that ANSWERED and holds no access slot is not a network
    // failure, and "check your connection and retry" is advice that can never succeed. It is an
    // orphan — the write never landed (or has not landed YET, which the error message covers). And
    // never "no wallet found for this passkey": the wallet is fine.
    const conn2 = createOwnOriginConnection({
      rpId: "qudi.fi",
      passkey,
