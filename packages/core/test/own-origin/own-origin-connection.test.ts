...
describe("createOwnOriginConnection.create", () => {
  ...
  it("account() returns { evm } after create", async () => {
    const passkey = makeFakePasskey();
...
describe("createOwnOriginConnection operatorName label", () => {
  ...
  // label the wallet module's create() ceremony receives — the string the OS/keychain shows the user.
...
describe("createOwnOriginConnection signing verbs", () => {
  ...
  // v0.9 userOpHash (what validateUserOp checks) and, when undelegated, the 7702 authorization.
