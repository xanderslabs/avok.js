 ...
 * user has to make. Counting it is the only honest way to test this, because a second prompt is
 * invisible in the results — the access slot is still written correctly, the user is just asked twice.
 *
 * Enrolment needs the wallet key TWICE (seal the access slot's blob under K, then sign the transaction
 * that carries it), and both reads MUST come from ONE gesture. Splitting them into two key scopes
 * would make adding ONE device ask for TWO confirmations. Beyond the annoyance, that trains people to
 * approve prompts reflexively — the exact habit a malicious second prompt relies on. These tests fail
 * the moment the count regresses.
 *
