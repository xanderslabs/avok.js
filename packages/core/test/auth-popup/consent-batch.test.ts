...
import { AvokCaliburABI, MODE_BATCH } from "@avokjs/contracts";
...
describe("the consent screen unwraps the wallet's own execute() batch", () => {
  it("shows the recipient and amount of a real send, not raw calldata", () => {
    ...
    const lines = displayText(formatConsentDisplay(consent));
    expect(lines.join("\n")).toContain(RECIPIENT);
...
/**
 * THE NATIVE-GAS CEILING IS DERIVED FROM THE SIGNED BYTES — nothing else is admissible here.
 *
 * A native-gas transaction commits no fee call, so this screen has no exact fee to decode. It does have
 ...
 */
describe("native-gas discloses the fee ceiling the signature commits to", () => {
  it("derives maxFeeWei from gas × maxFeePerGas on a signSend", () => {
    ...
    // Native-gas reimburses no one, so there must be no fee call masquerading as one.
 ...
 * must unwrap it exactly like a native-gas send — the paymaster paying the gas does not exempt the user
...
describe("signUserOp consent unwraps the UserOp callData and discloses delegation", () => {
  ...
  it("shows the recipient and amount from the UserOp batch, not raw calldata", () => {
    ...
    const lines = displayText(formatConsentDisplay(consent)).join("\n");
