import { isTerminalSigningFailure } from "../../src/auth-popup/ceremony.js";
...
describe("isTerminalSigningFailure", () => {
  // The question is only ever "can the user change this outcome by trying again".
  it("treats a dismissed or timed-out gesture as retryable, which is the whole point of staying open", () => {
    const cancelled = Object.assign(new Error("The operation either timed out or was not allowed."), {
      name: "NotAllowedError",
    });
    expect(isTerminalSigningFailure(cancelled)).toBe(false);
  });

  it("treats a relying-party mismatch as terminal, since the rpId is baked into the page", () => {
    const rp = Object.assign(
      new Error("The relying party ID is not a registrable domain suffix of, nor equal to the current domain."),
      { name: "SecurityError" },
    );
    expect(isTerminalSigningFailure(rp)).toBe(true);
  });

  it("matches the relying-party case on the message too, not only the error name", () => {
    expect(isTerminalSigningFailure(new Error("Relying Party ID mismatch"))).toBe(true);
  });

  // Stranding someone on a recoverable error is the worse mistake: Reject is on screen either way.
  it("defaults an unknown failure to retryable", () => {
    expect(isTerminalSigningFailure(new Error("network hiccup"))).toBe(false);
    expect(isTerminalSigningFailure(undefined)).toBe(false);
    expect(isTerminalSigningFailure("a string")).toBe(false);
  });
});
