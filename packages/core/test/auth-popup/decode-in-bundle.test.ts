...
// "The popup is self-contained" guards, anchored on the framework-free driver + wiring in
// src/auth-popup/ (mountAuthPopup). The BEHAVIOUR (reject-only on a failed decode, reply shapes,
// ready handshake) is pinned end-to-end in src/auth-popup/ceremony.test.ts; these are the cheap
// static guards that keep self-containment from regressing quietly.
...
describe("the sign popup is self-contained", () => {
  ...
  it("still signs in-page via performSign", () => {
    // Guard the money path: performSign is ALWAYS browser-side. Nothing may move it off the page.
