...
// FakePasskeyAdapter and FakeVaultReader live in the wallet module's test helpers.
...
describe("materializeWalletState", () => {
  ...
  it("throws an ORPHAN error when a SECONDARY's vault answers and has nothing — never 'no wallet', never 'retry'", async () => {
    // A chain that ANSWERED and holds no access slot must NOT raise SlotUnreachableError ("check your
    // connection and retry"). That conflates two different facts and leaves the user retrying a thing
