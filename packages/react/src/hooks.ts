 ...
 * (VISION §6 Surface 1), driven by stock wagmi/viem. The old useSend/useSimulate/useSign/useFeeTokens
 * hooks are gone.
 */
...
/**
 * Reveal the wallet's RAW private key. `exportEvmKey` restores the whole wallet: K IS the EVM key.
 * Each call runs a passkey gesture and returns raw hex, never a seed phrase. Gate this behind an
 * explicit "reveal keys" confirmation.
 */
 ...
 *
 * `remove(slotId, { confirm: true })` frees a slot (one gesture — a native-gas tx) and updates local state
