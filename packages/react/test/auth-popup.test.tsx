...
describe("<AuthPopup>", () => {
  ...
  it("renders the consent lines and resolves true on Approve", async () => {
    ...
    act(() => {
      decision = capturedView!.showConsent([{ text: "Sign message:", severity: "info" }, { text: "hello", severity: "info" }]);
    });
  ...
  it("a reject-only request offers only Close and resolves false", async () => {
    ...
    let decision: Promise<boolean>;
    act(() => {
      decision = capturedView!.showConsent([{ text: "Can't show this", severity: "info" }], { rejectOnly: true });
    });
