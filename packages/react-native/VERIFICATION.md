...
Both postures ship, so check both. `createOwnOriginConnection` covers the own-origin path;
`createNativeChannel` / `createNativeSharedOrigin` cover the shared-origin path over an in-app
browser session. See 3b below for the measurement that unblocked the second one.

### 3b. PRF inside an in-app browser tab (DONE — 2026-07-20)

**This section is the single source of truth for this measurement.** `README.md`,
`src/index.ts`, and `@avokjs/core`'s `channel/channels/native.ts` all point here rather than
restating the result. If it is re-run, update it here and nowhere else.
...
non-empty rather than `NoPrfError`.

**iOS floor is 18.4.** PRF itself landed earlier: WebKit's Safari 18.0 release notes state it
directly ("WebKit for Safari 18.0 adds support for the WebAuthn `prf` extension. It allows for
retrieving a symmetric key from a passkey to use for the encryption of user data"). But this rail
signs in an in-app browser tab, and cross-device (hybrid/QR) PRF was buggy through early 18.x and
only settled around 18.4. 18.0 is where PRF exists; **18.4 is where it can be relied on for the
flows this package actually drives**, so the floor is 18.4.

Two further limits are not version-gated and no floor removes them:

- PRF requires **iCloud Keychain to be enabled**. Without it there is no PRF, on any version.
- Apple does not pass PRF extension data to or from **external roaming authenticators** (security
  keys). Platform credentials only.
