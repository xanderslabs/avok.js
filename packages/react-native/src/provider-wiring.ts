 ...
 *
 * The ONE piece that is genuinely browser-specific is the ANNOUNCE — EIP-6963
 * (`window.dispatchEvent`). It is `window`-gated exactly as the browser wiring is: on RN-web
 * (`Platform.OS === "web"`) it fires and consumes `wallet`; on pure native there is no in-page
 ...
 * EIP-6963. On pure native the announce no-ops.
 *
...
export function createAvokClient<C extends Connection>(
  ...
  if (typeof window !== "undefined") {
    const icon = wallet?.icon ?? BLANK_ICON;
