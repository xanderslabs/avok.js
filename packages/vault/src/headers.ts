...
/**
 * The same policy as an Apache `.htaccess`, for operators on shared hosting (cPanel and friends),
 * where `_headers` means nothing and there is no server config to edit.
 *
 * Two things beyond a header dump matter here. First, the recovery route's COOP and COEP are applied
 * with an `<If>` block rather than a second file, because Apache merges `Header always set` across
 * matching sections and a later `set` wins, which is the behaviour the split relies on. Second, the
 * rewrite: the Vault is one page and `/recover/...` is a client-side route, so without it Apache
 * looks for a directory that was never uploaded and answers 404 at the exact moment a user is trying
 * to recover a wallet.
 */
export function htaccessContent(csp: string): string {
  const set = (headers: Record<string, string>, indent = "  "): string =>
    Object.entries(headers)
      .map(([name, value]) => `${indent}Header always set ${name} "${value.replace(/"/g, '\\"')}"`)
      .join("\n");

  const base = { "Content-Security-Policy": csp, ...securityHeaders() };
  const recoveryOnly = Object.fromEntries(
    Object.entries(recoverySecurityHeaders()).filter(([name]) => !(name in securityHeaders())),
  );

  return [
    "# Generated by avok-vault. Upload beside index.html at the Vault origin's document root.",
    "# Apache 2.4 with mod_headers and mod_rewrite enabled, which is the cPanel default.",
    "",
    "<IfModule mod_headers.c>",
    set(base),
    "",
    '  <If "%{REQUEST_URI} =~ m#^/recover(/|$)#">',
    set(recoveryOnly, "    "),
    "  </If>",
    "</IfModule>",
    "",
    ...
    "  RewriteRule ^recover(/.*)?$ /index.html [L]",
    "</IfModule>",
    "",
  ].join("\n");
}
