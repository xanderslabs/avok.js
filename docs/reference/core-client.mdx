...
| --- | --- | --- |
| `@avokjs/core` | `createAvokClient(config, wallet)` | A wired client with `getEip1193Provider()`, plus an announced EIP-6963 provider. |
...
| `paymasterUrl` | `string` | No | - | ERC-7677 paymaster for sponsored sends. Requires `bundlerUrl` too. |
| `bundlerUrl` | `string` | No | - | ERC-4337 bundler (EntryPoint v0.9) for sponsored sends. Requires `paymasterUrl` too. |
...
`Bundler` or `Paymaster7677` through it when a URL is not enough.
</Info>
...
<Warning>
The public client has no `.evm.send` method. Sending and signing go through the announced provider. See [The provider, not hooks](/concepts/provider-not-hooks).
</Warning>
