...
| `useExport()` | `{ exportEvmKey; pending: boolean; error: Error \| null }` | Self-custody only. |
...
| `SharedOrigin` | `{ auth: string; wallet: WalletInfo; children: ReactNode }`, plus optional `paymasterUrl?`, `bundlerUrl?`, `managementUrl?`, `fallback?`, `onError?` |
