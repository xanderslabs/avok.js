...
| `SponsorshipUnavailableError` | A send asked for sponsorship (a `feeToken`, or `sponsored`) and the rail is not reachable. Names which side (bundler or paymaster) is missing. Thrown before signing, so no passkey ceremony runs. There is no degrade. |
