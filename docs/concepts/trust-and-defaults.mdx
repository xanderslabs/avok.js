...
description: "Why Avok ships no default RPC or paymaster, and how the operator's identity is announced."
---
...
endpoints are rate-limited, carry no SLA, and block the indexed reads a wallet needs. Set `rpcUrls`
before production.
</Warning>
...
transaction you send, so Avok never defaults one. Supply `bundlerUrl` and `paymasterUrl` yourself.
See [Sponsorship](/concepts/sponsorship).
...
id that a dapp discovers in its wallet picker (over EIP-6963) are the **operator's**, supplied at
wiring time through the `wallet` argument to `createAvokClient`.
