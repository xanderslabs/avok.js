...
| Network | Chains |
| --- | --- |
| Mainnet | Ethereum, Optimism, Arbitrum, Base, BSC, Robinhood |
| Testnet | Arc, Ethereum Sepolia |

v1 is EVM-only. Wallets are smart EOAs through EIP-7702. To add a chain, you deploy the Avok contract
there and add a profile to `@avokjs/contracts`.

## You pick a chain per send

There is no "register your chains" step. Each send names its chain with `chainId`. The value must be
a registry chain, so an app opts into whichever chains it actually sends on by using their ids.
...
<Warning>
The public defaults are **development-only**. They are rate-limited, carry no SLA, and block the
indexed reads a wallet needs. Set `rpcUrls` before production.
</Warning>
