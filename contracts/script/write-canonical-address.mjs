#!/usr/bin/env node
// Writes the deployed canonical AvokCalibur address into the EVM entries of
// src-ts/registry.ts, replacing the `canonicalImplementation: PENDING`
// placeholders. The CREATE2 address is uniform across every EVM chain, so a single
// address is written to ALL EVM chains at once. (GuardianLogic, the other half of
// the canonical pair, is delegatecalled by AvokCalibur — it is never itself a 7702
// delegation target, so it has no registry slot of its own.)
//
...
/** Extract the AvokCalibur contract address from a forge broadcast artifact. */
export function addressFromBroadcast(path) {
  ...
  const create = txs.find(
    (t) =>
      ...
      t.contractName === "AvokCalibur" &&
      t.contractAddress,
  );
  if (!create) {
    throw new Error(`write-canonical-address: no AvokCalibur create tx in ${path}`);
  }
