#!/usr/bin/env node
// One-command maintainer CLI: deploys the canonical GuardianLogic + AvokCalibur pair
// (via the two-step CREATE2 deploy in DeployCanonical.s.sol) to a chain and, on
// success, writes AvokCalibur's deterministic address into src-ts/registry.ts.
// GuardianLogic is never itself a 7702 delegation target (AvokCalibur delegatecalls
// into it), so it has no registry slot and this CLI does not track its address.
//
...
// Must match GOLDEN_AVOK_CALIBUR in test/DeployCanonical.t.sol and the require() in
// DeployCanonical.s.sol. This is the CREATE2 address of the CURRENT AvokCalibur bytecode
// (constructor arg = GuardianLogic's own CREATE2 address) under the canonical salt via
// the Arachnid deployer (0x4e59…). GuardianLogic itself has an analogous golden constant
// (GOLDEN_GUARDIAN_LOGIC in the same test file) but no registry slot, since it is never a
// 7702 delegation target.
//
// It drifted once, pre-guardian-contracts (a stale address survived a bytecode change while
// the Solidity test + registry moved on) and NOTHING caught it — each suite only checked its
// own copy. golden-drift.test.ts now cross-checks this constant against the Solidity literal.
export const GOLDEN = "0x1a29eF50E033371d9686F027BD7d0743B1A0Cc3e";

// Every chain this CLI can deploy to. A name here MUST have a matching profile in src-ts/registry.ts,
// because a successful deploy writes the address into that profile's canonicalImplementation slot.
export const CHAIN_IDS = {
  ...
  arbitrum: 42161,
  arc: 5042002,
  // TESTNET. The registry's only EVM testnet, and so the only chain a sponsored send can be proven on
