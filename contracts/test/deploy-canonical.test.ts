...
describe("deploy-canonical chain map + arg parsing (pure)", () => {
  ...
  it("every deployable chain has a registry profile to write the address INTO", () => {
    // A successful deploy writes the canonical address into that chain's `canonicalImplementation`
    // slot. A name in CHAIN_IDS with no matching profile would deploy the delegate for real and then
    // have nowhere to record it — the CLI would report success while the SDK still believed the chain
    // had no implementation. Cheap to add a name, silent to get wrong, so it is pinned here.
    const known = new Set(Object.values(CHAIN_PROFILES).map((p) => (p as { chainId?: number }).chainId));
    for (const [name, chainId] of Object.entries(CHAIN_IDS)) {
      expect(known.has(chainId), `CHAIN_IDS.${name} (${chainId}) has no registry profile`).toBe(true);
    }
  });
